...
int add_node(List **list, char *content)
{
  ...
  /* Make the first node 'next' ptr point to the first element of node (by accessing its addr) */
  (*list)->next = node;
  /* Make ptr 'next' of 2nd node point to NULL, so the 2nd node will now be the last node of our list */
  node->next = NULL;
  return(0);
}
